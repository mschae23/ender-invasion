package de.martenschaefer.enderinvasion.worldgen;
...
public class EnderInvasionPlacer {
 ...
 protected static boolean generateVeinPart(WorldAccess worldAccess, Random random, int patchSize, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int size, int i) {
  ...
  double[] ds = new double[patchSize * 4];
  ...
  double q;
  double r;
  for(m = 0; m < patchSize; ++m) {
   float f = (float)m / (float)patchSize;
   o = MathHelper.lerp(f, startX, endX);
   p = MathHelper.lerp(f, startY, endY);
   q = MathHelper.lerp(f, startZ, endZ);
   r = random.nextDouble() * (double)patchSize / 16.0D;
